...
The map loads the MapTiler Dataviz Dark MapLibre style (`dataviz-v4-dark`) from the Maps API using the local-only `config.local.json` key; the normal compact MapLibre attribution control and a visible MapTiler logo remain enabled. MapLibre handles desktop and touch navigation, resize, DPR, and the map WebGL context; camera zoom is constrained to raw MapLibre zoom 1.5 or higher. The custom weather layer is attached from the style-ready lifecycle (`style.load`) with an idempotent `getLayer` check. The MapTiler-specific context step leaves the native basemap visually unmodified below weather while moving selected geographic and water labels plus verified administrative boundaries above it. One low-opacity derived water fill sits above weather to make water areas legible; no custom water outlines or edge-shadow layers are used. Unavailable optional context layers are skipped without preventing weather initialization. MapLibre error events remain visible in the console without exposing the authenticated style URL.
