...
`geographic-symbol-pyramid.js` caches the L8–L15 samples and their immediate dyadic ownership. L15 evaluates the reconstructed geographic field and derives final ordinary-rain radius, strong-rain radius, hazard type, and hazard radius with the normal mappings. Every lower level is then reduced only from the already-built next finer representation. A fine grid vertex owns exactly one parent chosen by canonical Mercator coordinates with `floor` ownership; interior parents own their stable 2×2 child group, while support-edge groups are deterministically partial.

The lattice remains vertex-based rather than cell-centered: a parent is anchored at one corner of its owned 2×2 child block. The recursive ownership fixes grouping semantics without changing that pre-existing topology characteristic.
...
This produces a deterministic multiresolution symbol pyramid: L14 and L15 both carry finer reconstructed storm/hail symbols; L13 remains the useful provider-scale reference rather than a hard hazard ceiling; and L12 through L8 are recursive reductions rather than direct reassignment from an arbitrary source level.
