...
`geographic-symbol-pyramid.js` caches the L9–L15 samples, immediate dyadic ownership below L13, and static render anchors. L13 is evaluated directly as the provider-scale reference. L12 through L9 recursively reduce only from the immediately finer reference representation required for the current display or adjacent transition. L14 and L15 independently evaluate the reconstructed geographic field directly; neither needs L15 as a source for coarser levels. A fine grid vertex below L13 owns exactly one parent chosen by canonical Mercator coordinates with `floor` ownership; interior parents own their stable 2×2 child group, while support-edge groups are deterministically partial.

Topology identity and render position are deliberately separate for reduced symbols. The parent retains its canonical lattice ID, while its cached render anchor is the unweighted geometric mean of its immediate child anchors. This is applied recursively from L13 down to L9, including partial support-edge groups. Anchors are static topology data, never weighted by weather values, so animation cannot move a representation's spatial identity.

Rain and strong precipitation conserve their visible areas independently: each parent radius is `sqrt(sum(child radius²))`. Hazards separate priority from footprint. A parent is hail if any child is hail, otherwise storm if any child is storm, while its radius preserves the sum of all child hazard painted areas. The hail hexagon coefficient is `3√3/2`; the current eight-point storm star coefficient is `2 * 0.38 * √2`, derived from its alternating outer/inner polygon geometry. The winning parent glyph radius is converted from the combined area using its own coefficient.
