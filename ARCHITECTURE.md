...
Normal camera interaction keeps the native raw MapLibre minimum at `1.5`. The raw `maxZoom` is derived from the fixed logical boundary before L15 would be selected: `MAX_LOGICAL_SAMPLING_ZOOM = 14.5 - log2(512 / 9)` (approximately `8.669925`) plus the current-latitude adjustment relative to `WEATHER_REGION.center[1]`. Displayed weather LOD is capped at L14, while L15 remains available to the canonical grid and internal symbol-pyramid topology.
