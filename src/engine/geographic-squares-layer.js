...
void main() {
  ...
  float squall = strength(v_values.w, ${INTENSITY_THRESHOLDS.squall.toFixed(3)});
  float hurricane = strength(v_hurricane, ${INTENSITY_THRESHOLDS.hurricane.toFixed(3)});
  if (hurricane > 0.0) { color = vec3(0.86, 0.015, 0.08); alpha = max(alpha, hurricane); }
  else if (squall > 0.0) {
    color = v_values.w >= ${SQUALL_GRADE_THRESHOLDS[2].toFixed(3)} ? vec3(0.95, 0.07, 0.02)
      : v_values.w >= ${SQUALL_GRADE_THRESHOLDS[1].toFixed(3)} ? vec3(1.0, 0.24, 0.0)
        : vec3(1.0, 0.52, 0.0);
    alpha = max(alpha, squall);
  } else if (hail > 0.0) { color = mix(color, vec3(1.0, 0.831, 0.0), mix(0.5, 1.0, pow(hail, 0.47))); alpha = max(alpha, hail); }
  else if (storm > 0.0) { color = mix(color, vec3(1.0, 0.0, 1.0), mix(0.45, 1.0, pow(storm, 0.47))); alpha = max(alpha, storm); }
  ...
  const names = ['a_vertex', 'a_center', 'a_values0', 'a_hurricane0', 'a_values1', 'a_hurricane1', 'u_temporalProgress', 'u_spacing', 'u_opacity', 'u_matrix', 'u_projection_fallback_matrix', 'u_projection_matrix', 'u_projection_tile_mercator_coords', 'u_projection_clipping_plane', 'u_projection_transition'];
...
function makeState(length, reusable) {
  ...
  return { rain: new Float32Array(length), storm: new Float32Array(length), hail: new Float32Array(length), squall: new Float32Array(length), hurricane: new Float32Array(length) };
}
  ...
  evaluate(levels, frame, reusable = null) {
    ...
    if (wantsReduced) {
      ...
      for (let index = 0; index < reference.samples.length; index++) {
        ...
        direct.rain[index] = value.rain; direct.storm[index] = value.storm; direct.hail[index] = value.hail; direct.squall[index] = value.squall; direct.hurricane[index] = value.hurricane;
      }
      ...
      for (let level = REFERENCE_GRID_LEVEL - 1; level >= minimum; level--) {
        ...
        for (let parentIndex = 0; parentIndex < parent.samples.length; parentIndex++) {
          ...
          let rain = 0, storm = 0, hail = 0, squall = 0, hurricane = 0, stormMax = 0, hailMax = 0, squallMax = 0, hurricaneMax = 0;
          for (const child of indices) {
            rain += children.rain[child]; storm += children.storm[child]; hail += children.hail[child]; squall += children.squall[child]; hurricane += children.hurricane[child];
            stormMax = Math.max(stormMax, children.storm[child]); hailMax = Math.max(hailMax, children.hail[child]); squallMax = Math.max(squallMax, children.squall[child]); hurricaneMax = Math.max(hurricaneMax, children.hurricane[child]);
          }
          ...
          state.squall[parentIndex] = (squall / indices.length) * 0.30 + squallMax * 0.70;
          state.hurricane[parentIndex] = (hurricane / indices.length) * 0.18 + hurricaneMax * 0.82;
        }
        states[level] = state;
        children = state;
      }
    ...
    for (const level of requested) {
      ...
      for (let index = 0; index < entry.samples.length; index++) {
        ...
        state.rain[index] = value.rain; state.storm[index] = value.storm; state.hail[index] = value.hail; state.squall[index] = value.squall; state.hurricane[index] = value.hurricane;
      }
      states[level] = state;
    }
    return states;
...
export class GeographicSquaresLayer {
  ...
  buildGroup(group, level, state0, state1) {
    ...
    for (let index = 0, offset = 0; index < samples.length; index++, offset += INSTANCE_STRIDE) {
      ...
      result[offset + 2] = state0.rain[index]; result[offset + 3] = state0.storm[index]; result[offset + 4] = state0.hail[index]; result[offset + 5] = state0.squall[index]; result[offset + 6] = state0.hurricane[index];
      result[offset + 7] = state1.rain[index]; result[offset + 8] = state1.storm[index]; result[offset + 9] = state1.hail[index]; result[offset + 10] = state1.squall[index]; result[offset + 11] = state1.hurricane[index];
    }
  ...
  renderGroup(gl, entry, projection, group, opacity) {
    ...
    gl.vertexAttribPointer(locations.a_hurricane0, 1, gl.FLOAT, false, INSTANCE_STRIDE * 4, 24); gl.vertexAttribDivisor(locations.a_hurricane0, 1);
    gl.enableVertexAttribArray(locations.a_values1);
    gl.vertexAttribPointer(locations.a_values1, 4, gl.FLOAT, false, INSTANCE_STRIDE * 4, 28); gl.vertexAttribDivisor(locations.a_values1, 1);
    gl.enableVertexAttribArray(locations.a_hurricane1);
    gl.vertexAttribPointer(locations.a_hurricane1, 1, gl.FLOAT, false, INSTANCE_STRIDE * 4, 44); gl.vertexAttribDivisor(locations.a_hurricane1, 1);
    gl.drawArraysInstanced(gl.TRIANGLES, 0, 6, this.instanceCounts[group]);
    for (const location of [locations.a_center, locations.a_values0, locations.a_hurricane0, locations.a_values1, locations.a_hurricane1]) gl.vertexAttribDivisor(location, 0);
  }
