...
function frameValue(store, gx, gy, frame) {
  const tileX = tileFor(gx); const tileY = tileFor(gy); const tileKey = key(tileX, tileY);
  ...
  const localX = localFor(gx) + (store.motionWarp ? TILED_RAIN_WARP_HALO_SIZE : 0);
  const localY = localFor(gy) + (store.motionWarp ? TILED_RAIN_WARP_HALO_SIZE : 0);
...
export function bilinearRain(store, gx, gy, frame) {
  ...
  for (const [x, y, weight] of taps) {
    const tap = frameValue(store, x, y, frame);
...
function motionNode(store, nodeX, nodeY, interval) {
  const tileKey = key(tileFor(nodeX), tileFor(nodeY));
...
export function interpolateMotion(store, gx, gy, interval) {
  const tileX = tileFor(gx); const tileY = tileFor(gy);
  const localX = localFor(gx); const localY = localFor(gy);
  ...
  for (let row = 0; row < 2; row++) for (let column = 0; column < 2; column++) {
    ...
    const node = motionNode(store, (tileX * 2 + lowerX + column) * 64, (tileY * 2 + lowerY + row) * 64, interval);
...
function directRain(store, gx, gy, frameA, frameB, progress) {
  const a = frameValue(store, gx, gy, frameA); const b = frameValue(store, gx, gy, frameB);
...
export function evaluateTiledRainSample(store, gx, gy, frame, { debugMode = store.motionWarpDebugMode } = {}) {
  ...
  const direct = directRain(store, gx, gy, frameA, frameB, progress);
  const interval = Math.min(frameA, (store.motionManifest?.interval_count || 1) - 1);
  const motion = store.motionWarp ? interpolateMotion(store, gx, gy, interval) : { dx: 0, dy: 0, confidence: 0, nodes: [] };
  ...
  const warpedA = canWarp ? bilinearRain(store, gx - motion.dx * progress, gy - motion.dy * progress, frameA) : { available: false, value: 0, taps: [], reason: 'warp-not-evaluated' };
  const warpedB = canWarp ? bilinearRain(store, gx + motion.dx * (1 - progress), gy + motion.dy * (1 - progress), frameB) : { available: false, value: 0, taps: [], reason: 'warp-not-evaluated' };
  ...
  const present = (value) => ({ mmh: value, radiusFraction: rainMmhToRadiusFraction(value), visibility: value > 0, strongRadiusFraction: dotsStrongRainMmhToRadiusFraction(value), strongVisibility: value > 1.6 });
  return { frameA, frameB, progress, interval, endpointRain: { codeA: direct.a.available ? direct.a.code : null, codeB: direct.b.available ? direct.b.code : null, validA: direct.validA, validB: direct.validB, decodedA: direct.rainA, decodedB: direct.rainB, direct: direct.value }, motion, warpA: { coordinate: [gx - motion.dx * progress, gy - motion.dy * progress], ...warpedA }, warpB: { coordinate: [gx + motion.dx * (1 - progress), gy + motion.dy * (1 - progress),], ...warpedB }, warpedTemporal: warpedAvailable ? warped : null, final: present(final), usedDirectFallback: usedFallback, fallbackReason, motionWarpDebug: debugMode || null, presentation: { base: present(final), strong: { radiusFraction: dotsStrongRainMmhToRadiusFraction(final), visibility: final > 1.6 }, baseMaxRadiusFraction: DOTS_BASE_RAIN_MAX_RADIUS_FRACTION } };
}
...
export function buildMotionProbe(store, sample, selectedFrame, currentFrame, { timestamps = [], longitude = null, latitude = null, selectedAtTimestamp = null, currentTimestamp = null } = {}) {
  ...
  return finiteJson({ schema: MOTION_PROBE_SCHEMA, dataset: { sourceGenerationId: store.manifest.source_generation_id, rainManifest: store.dataset?.rainManifestUrl || (store.motionWarp ? null : store.dataset?.manifestUrl) || null, motionManifest: store.motionManifestUrl || null, warpManifest: store.motionWarp ? store.dataset?.manifestUrl || null : null }, sample: { globalL13: { x: sample.x, y: sample.y }, owningRainTile: { x: tileFor(sample.x), y: tileFor(sample.y) }, tileLocalCore: { x: localFor(sample.x), y: localFor(sample.y) }, longitude, latitude }, selectedAt: { ...stateObject(store, selectedFrame, timestamps), timestamp: selectedAtTimestamp, evaluation: evaluate(selectedFrame) }, current: { ...stateObject(store, currentFrame, timestamps), timestamp: currentTimestamp, evaluation: current }, motionTrace: motionTrace(store, sample.x, sample.y, timestamps), temporalSweep: sweep, neighborhood, diagnosticConstants: { motionGridNodeSpacingL13Samples: 64, motionDisplacementBoundL13Samples: 12, largeVectorChangeThresholdSamples: MOTION_PROBE_LARGE_VECTOR_CHANGE, sweepSteps: 21, nodataCode: 0, dryCode: 1, physicalMaxMmh: store.manifest.encoding.physical_max_mmh } });
}
