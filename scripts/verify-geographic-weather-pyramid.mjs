...
function maxSummaryDifference(left, right) {
  ...
  for (let threshold = 0; threshold < RAIN_COVERAGE_THRESHOLDS_MMH.length; threshold++) for (let index = 0; index < left.levelData.count; index++) error = max(error, Math.abs(left.rainCoverageWeight[threshold][index] - right.rainCoverageWeight[threshold][index]));
  return error;
}
...
function directRefinementCheck(lowerLevel, higherLevel) {
  ...
  for (let index = 0; index < higher.levelData.count; index++) {
    ...
    if (indices.every((candidate) => candidate >= 0)) {
